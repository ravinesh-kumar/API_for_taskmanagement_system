            ...
            if (error)
                ...
                try {
                    ...
                    mailOptions = {
                        ...
                        to: data.email,
                        subject: "Account is Created : By Task Management PVT. LTD.",
                        text: `
                                Hello ${data.username}
                                Your Account is Successfully Created 
                                Now You Can Manage Your Task Easily.
                                Team : Task Management PVT. LTD.
                            `
                    }
    ...
    try {
        ...
        if (data) {
            if (await bcrypt.compare(req.body.password, data.password)) {
                ...
                res.send({ status: 401, result: "Fail", message: "1 Invalid Username or Password" })
        }
        else
            res.send({ status: 401, result: "Fail", message: "2 Invalid Username or Password" })
    } catch (error) {
        // console.log(data);
        console.log(error);
        res.send({ status: 500, result: "Fail", message: "Internal Server Error" })
